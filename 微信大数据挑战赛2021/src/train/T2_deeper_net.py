...
def main(argv):
    ...
    warm_up_lr = WarmUpLearningRateScheduler(batch_size, init_lr=lr)
    ...
    train_model.save_weights(MODEL_PATH + 'mmoe_deeper_seed_' + str(SEED) + '.h5')
    ...
    try:
        submit.to_csv(SUB_PATH + 'deeper_SEED_%.6f_sub_%.6f_%.6f_%.6f_%.6f.csv' % (
        SEED, all_uAUC[0], all_uAUC[1], all_uAUC[2], all_uAUC[3]),
