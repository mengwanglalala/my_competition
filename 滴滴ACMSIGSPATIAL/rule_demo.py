...
def get_link_fea(links, crosses):
    ...
    for link in links:
        ...
        if link_id not in all_topo_link.keys():
            ...
            mp.append(link_nums) #记录每个link出现的次数
        else:
    ...
    sum_cross_time = 0
    try:
        for cross in crosses: #没有统计路口数量！！！！！！！！！！！！！！！！！！！需要改
            cross_fe = cross.split(':')
            sum_cross_time += int(cross_fe[1]) #考虑十字路口的时间
    except:
        pass
    return  maps,sum_link_time,sum_link_time_w, sum_cross_time, sum_stut
...
for i in tqdm(range(len(df_test_head))):
    link, t_t, w_t, c_t, s_t = get_link_fea(df_test_link.loc[[i]].values[0], df_test_cross.loc[[i]].values[0])
...
for file in tqdm(filenames):
    ...
    for i in range(len(df_train_head)):
        link, t_t, w_t, c_t, s_t = get_link_fea(df_train_link.loc[[i]].values[0], df_train_cross.loc[[i]].values[0])
