...
test_data = pd.read_feather('../../../wbdc2021/data/wedata/wechat_algo_data2/test_a.csv')
...
train_model = AutoInt(linear_feature_columns, dnn_feature_columns, att_layer_num=6, att_embedding_size=8,
                      att_head_num=6, dnn_hidden_units=(1024, 512, 128),
                      seed=2021)  # task_dnn_units= (1024, 512, 128), #可能比例越低的目标需要更深的网络来提取
...
for z in seed:
    ...
    for i, y in enumerate(target):
        if (y == 'read_comment' or y == 'like' or y == 'click_avatar'):
            epoch = 4
            train_model = AutoInt(linear_feature_columns, dnn_feature_columns, att_layer_num=6, att_embedding_size=8,
                                  att_head_num=6, dnn_hidden_units=(1024, 512, 128), seed=z)
            train_model.compile(Adagrad(0.05), loss='binary_crossentropy')
            warm_up_lr = WarmUpLearningRateScheduler(batch_size, init_lr=0.02)
        elif (y == 'click_avatar'):
            epoch = 4
            train_model = AutoInt(linear_feature_columns, dnn_feature_columns, att_layer_num=6, att_embedding_size=8,
                                  att_head_num=8, dnn_hidden_units=(2048, 1024, 512),
            ...
            epoch = 1
            train_model = AutoInt(linear_feature_columns, dnn_feature_columns, att_layer_num=6, att_embedding_size=8,
                                  att_head_num=6, dnn_hidden_units=(2048, 1024, 512),
                                  seed=z)  # task_dnn_units= (1024, 512, 128), #可能比例越低的目标需要更深的网络来提
            train_model.compile(Adagrad(0.01), loss='binary_crossentropy')
            warm_up_lr = WarmUpLearningRateScheduler(batch_size, init_lr=0.04)
        else:
        ...
        train_model.save_weights(MODEL_PATH + 'AutoInt_allfeature_' + str(z) + '_' + str(val_uauc) + '_' + y + '.h5')
        ...
        submit[y] = pred_ans
    # print('uauc:',all_uAUC[0]*(4/13)+all_uAUC[1]*(3/13)+all_uAUC[2]*(2/13)+all_uAUC[3]*(1/13)+all_uAUC[4]*(1/13)+all_uAUC[5]*(1/13)+all_uAUC[6]*(1/13))
    # zlx = all_uAUC[0]*(4/13)+all_uAUC[1]*(3/13)+all_uAUC[2]*(2/13)+all_uAUC[3]*(1/13)+all_uAUC[4]*(1/13)+all_uAUC[5]*(1/13)+all_uAUC[6]*(1/13)
    SUB_PATH = './result/'
    try:
        submit.to_csv(SUB_PATH + 'AutoInt_sub_' + str(z) + '.csv',
